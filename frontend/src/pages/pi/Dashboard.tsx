// pages/pi/PIDashboard.tsx â€” WITH REJECTED REQUESTS + FILE NUMBER + VIEW FILE
...
const fmtINR=(n:number)=>"â‚¹"+parseFloat(String(n||0)).toLocaleString("en-IN",{maximumFractionDigits:0});
const fmtINRFull=(n:number)=>{const v=parseFloat(String(n||0));if(v>=10000000)return`â‚¹${(v/10000000).toFixed(2)} Cr`;if(v>=100000)return`â‚¹${(v/100000).toFixed(2)} L`;return"â‚¹"+v.toLocaleString("en-IN");};
      ...
      <div style={{background:"linear-gradient(135deg, #0f2044 0%, #1a3a6e 60%, #1e4080 100%)"}}>
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center justify-between py-2 border-b border-white/10">
            <p className="text-[11px] text-blue-200 font-medium tracking-wide uppercase">IFMS â€” Integrated Financial Management System</p>
          ...
          <div className="flex items-center justify-between py-5 gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-blue-300 text-xs mt-1 font-medium">Principal Investigator &nbsp;Â·&nbsp; {PI_DEPT}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-4 gap-px bg-white/10 rounded-t-xl overflow-hidden">
            {[
              ...
              {label:"Amount Booked",value:fmtINRFull(totalBooked),sub:totalReleased>0?`${((totalBooked/totalReleased)*100).toFixed(0)}% utilised`:"â€”",icon:<FileText className="h-4 w-4"/>,accent:"text-purple-200"},
              {label:"Actual Expenditure",value:fmtINRFull(totalActual),sub:totalBooked>0?`${((totalActual/totalBooked)*100).toFixed(0)}% of booked`:"â€”",icon:<BarChart3 className="h-4 w-4"/>,accent:"text-green-200"},
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-5">
        ...
        {openQueries>0&&(
          <div className="flex items-center gap-4 bg-white border-l-4 border-amber-500 rounded-r-xl shadow-sm px-5 py-4">
            ...
            <div className="flex-1"><p className="text-sm font-bold text-gray-900">Action Required â€” {openQueries} Query{openQueries>1?" Responses":" Response"} Pending</p><p className="text-xs text-gray-500 mt-0.5">Your budget request(s) are on hold. Respond to queries to proceed.</p></div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between gap-4 pt-1">
          ...
          <div className="relative w-64">
            ...
            <Input placeholder="Search by GP No. or project nameâ€¦" value={projectSearch} onChange={e=>setProjectSearch(e.target.value)} className="pl-9 h-9 text-sm bg-white border-gray-200 rounded-lg shadow-sm"/>
          </div>
        </div>

        {loadingP?(
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-gray-200 shadow-sm">
            <Loader2 className="h-8 w-8 animate-spin text-blue-300 mb-3"/><p className="text-sm text-gray-400 font-medium">Loading project dataâ€¦</p>
          </div>
        ):filtered.length===0?(
                    ...
                    <div className="flex items-start justify-between gap-6 mb-4">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-1.5 flex-wrap">
                          ...
                          <span className="inline-flex items-center gap-1.5 text-xs text-gray-500"><Calendar className="h-3.5 w-3.5"/>{fmtDate(p.projectStartDate)} â€” {fmtDate(p.projectEndDate)}</span>
                      ...
                      :activeReqs.length===0?(
                        ...
                        <div>
                          <div className="grid grid-cols-[1.5fr_2.5fr_1fr_1fr_1.2fr_1.8fr_110px] text-[10px] font-bold uppercase tracking-widest text-gray-400 bg-gray-50 border-b border-gray-100 px-6 py-2.5 gap-0">
                            <span>Head / File No.</span><span>Purpose</span><span>Date</span><span className="text-right">Booked (â‚¹)</span><span className="text-right">Actual (â‚¹)</span><span className="text-center">Status</span><span></span>
                          </div>
                            ...
                            return(
                              <div key={req.id}>
                                <div className={`grid grid-cols-[1.5fr_2.5fr_1fr_1fr_1.2fr_1.8fr_110px] px-6 py-4 items-center gap-0 ${!isLast?"border-b border-gray-100":""} ${hasQuery?"bg-amber-50/40":"hover:bg-gray-50/50"} transition-colors`}>
                                  <div><p className="text-sm font-semibold text-gray-800">{req.headName||"â€”"}</p>{req.fileNumber&&<p className="text-[11px] font-mono font-bold text-blue-600 mt-0.5">{req.fileNumber}</p>}{req.invoiceNumber&&<p className="text-[11px] font-mono text-gray-400 mt-0.5">{req.invoiceNumber}</p>}</div>
                                  <p className="text-xs text-gray-600 leading-relaxed line-clamp-2">{req.purpose||"â€”"}</p>
                          ...
                          {activeReqs.length>1&&(<div className="grid grid-cols-[1.5fr_2.5fr_1fr_1fr_1.2fr_1.8fr_110px] px-6 py-3 bg-slate-50 border-t-2 border-slate-200 items-center gap-0"><div className="col-span-3 text-[11px] font-bold text-gray-500 uppercase tracking-widest">Project Total</div><div className="text-right"><p className="text-sm font-bold text-gray-900 font-mono">{fmtINR(activeReqs.reduce((s,r)=>s+parseFloat(String(r.amount||0)),0))}</p></div><div className="text-right"><p className="text-sm font-bold text-emerald-700 font-mono">{actual>0?fmtINR(actual):<span className="text-gray-300">â€”</span>}</p></div><div/><div/></div>)}
                        </div>
                      )}
                    </div>
                  )}

                  {/* â”€â”€ REJECTED REQUESTS SECTION â”€â”€ */}
                      ...
                      {rejOpen&&(
                        <div className="border-t border-red-100">
                          ...
                          <div className="grid grid-cols-[1.5fr_2fr_1.2fr_1fr_1.5fr_2fr_110px] text-[10px] font-bold uppercase tracking-widest text-red-400 bg-red-50/60 border-b border-red-100 px-6 py-2.5 gap-0">
                            <span>Head / File No.</span><span>Purpose</span><span>Rejected On</span><span className="text-right">Amount (â‚¹)</span><span>Rejected By</span><span>Reason for Rejection</span><span className="text-center">Quotation</span>
                          </div>
                          {rejectedList.map((req,i)=>{
                            const amount=parseFloat(String(req.amount||0));
                            const isLast=i===rejectedList.length-1;
                            const reason=req.rejectionRemarks||req.daRemarks||req.arRemarks||req.drRemarks||req.drcOfficeRemarks||req.drcRcRemarks||req.drcRemarks||req.directorRemarks||"â€”";
                            return(
                              <div key={req.id} className={`grid grid-cols-[1.5fr_2fr_1.2fr_1fr_1.5fr_2fr_110px] px-6 py-4 items-start gap-0 ${!isLast?"border-b border-red-100":""} bg-red-50/10 hover:bg-red-50/30 transition-colors`}>
                                {/* Head + file number */}
                                <div>
                                  <p className="text-sm font-semibold text-gray-700 leading-snug">{req.headName||"â€”"}</p>
                                  {req.fileNumber
                                ...
                                <p className="text-xs text-gray-600 leading-relaxed line-clamp-3 pt-0.5">{req.purpose||"â€”"}</p>
                                ...
                                {/* Rejected by */}
                                <div>
                                  <p className="text-xs font-bold text-red-700 leading-snug">{req.rejectedAtStageLabel||req.rejectedAtStage||"â€”"}</p>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200">
          <p className="text-[11px] text-gray-400">IFMS Portal â€” Integrated Financial Management System</p>
