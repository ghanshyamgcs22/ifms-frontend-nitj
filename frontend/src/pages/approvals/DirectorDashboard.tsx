...
// ── Split button: "Query to PI" + "Send Back to DRC" under one trigger ────────
const SendBackQueryButton = ({
  ...
  return (
    <div ref={ref} className="relative inline-flex">
      {/* Primary half — defaults to Query PI (most common) */}
      <button
...
const DirectorDashboard = () => {
  ...
  const actionBtnLabel = () => {
    if (pendingAction === "approve")  return "Final Approve ✓";
          ...
          <div className="bg-white/70 backdrop-blur-lg border border-violet-200/60 rounded-xl p-6 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                ...
                &nbsp;<span className="text-slate-400 font-mono text-[11px]">… → DRC (R&C) → DRC → <strong className="text-violet-600">Director</strong></span>
              </p>
            </div>
      ...
      <TableBody>
        {requests.map(r => (
          <TableRow key={r.id} className={`border-b border-slate-100 hover:bg-slate-50/50 ${canAct(r) ? "bg-violet-50/40" : ""}`}>
            ...
            <TableCell className="text-xs px-3 text-slate-500 max-w-[100px] truncate italic">{r.drcRemarks || "—"}</TableCell>
