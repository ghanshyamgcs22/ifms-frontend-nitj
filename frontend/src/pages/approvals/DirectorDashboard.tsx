...
// "Query to PI" and "Send Back to DRC" merged into one split-button dropdown
...
import { Search, Clock, CheckCircle, Loader2, Lock, XCircle, RotateCcw, Eye, FileText, ChevronDown, MessageSquare, CornerUpLeft } from "lucide-react";
...
type ActionType = "approve" | "query" | "sendback" | null;
...
// â”€â”€ Split button: "Query to PI" + "Send Back to DRC" under one trigger â”€â”€â”€â”€â”€â”€â”€â”€
const SendBackQueryButton = ({
  onQuery, onSendBack,
}: { onQuery: () => void; onSendBack: () => void }) => {
  ...
  return (
    <div ref={ref} className="relative inline-flex">
      {/* Primary half â€” defaults to Query PI (most common) */}
      <button
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute top-8 left-0 z-50 min-w-[190px] bg-white border border-slate-200 rounded-lg shadow-lg py-1 overflow-hidden">
          <button
            onClick={() => { setOpen(false); onQuery(); }}
...
const DirectorDashboard = () => {
  ...
  const handleAction = async () => {
    ...
    if (pendingAction !== "approve" && !remarks.trim()) { toast.error("Please enter remarks"); return; }
    try {
  ...
  const dialogTitle = () => {
    if (pendingAction === "approve")  return "Final Approval â€” Director";
  ...
  const actionBtnLabel = () => {
    if (pendingAction === "approve")  return "Final Approve âœ“";
    if (pendingAction === "query")    return "Send Query to PI";
    if (pendingAction === "sendback") return "Send Back to DRC";
    return "";
  };
  const actionBtnCls = () => {
    if (pendingAction === "approve")  return "bg-violet-700 hover:bg-violet-800";
          ...
          <div className="bg-white/70 backdrop-blur-lg border border-violet-200/60 rounded-xl p-6 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                Final approval for requests <strong className="text-violet-600">&gt; â‚¹25,000</strong>
                &nbsp;<span className="text-slate-400 font-mono text-[11px]">â€¦ â†’ DRC (R&C) â†’ DRC â†’ <strong className="text-violet-600">Director</strong></span>
              </p>
            </div>
            ...
            <CardHeader className="border-b border-slate-100">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription className="mt-0.5">Only <strong>DRC-forwarded</strong> requests appear here. This is the final stage.</CardDescription>
                </div>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedRequest && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-violet-50 rounded-xl border border-violet-200">
                {[
              ...
              <div className="space-y-2">
                ...
                {selectedRequest.drcRemarks       && <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-lg"><p className="text-xs text-indigo-600 font-medium mb-1">DRC Remarks:</p>         <p className="text-sm text-slate-700">{selectedRequest.drcRemarks}</p></div>}
              </div>
              ...
              {pendingAction && canAct(selectedRequest) && (
                <div className="space-y-1.5">
                  <Label className="text-sm">
                    {pendingAction === "approve" ? "Director Remarks (Optional)" : "Remarks (Required)"}
                  </Label>
                  <Textarea
                    placeholder={
                      pendingAction === "query"    ? "Describe the clarification needed from PIâ€¦"
                      : pendingAction === "sendback" ? "Reason for returning to DRCâ€¦"
                      : "Final remarks (optional)â€¦"
                    }
                    value={remarks} onChange={e => setRemarks(e.target.value)}
                    rows={3} className="border-violet-200 focus:border-violet-400"
                  />
                </div>
              )}
              ...
              {!pendingAction && canAct(selectedRequest) && (
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <Button size="sm" className="h-8 text-xs bg-violet-700 hover:bg-violet-800" onClick={() => setPendingAction("approve")}>Final Approve</Button>
                    ...
                    <Button size="sm" variant="outline" className="h-8 text-xs border-orange-400 text-orange-600 hover:bg-orange-50" onClick={() => setPendingAction("sendback")}>Send Back to DRC</Button>
                  </div>
                </div>
              )}
    ...
    <Table>
      <TableHeader>
        <TableRow className="bg-slate-50">
          {["Date", "GP Number", "PI Name", "Purpose", "Amount", "DRC Remarks", "Actions"].map(h => (
      ...
      <TableBody>
        {requests.map(r => (
          <TableRow key={r.id} className={`border-b border-slate-100 hover:bg-slate-50/50 ${canAct(r) ? "bg-violet-50/40" : ""}`}>
            ...
            <TableCell className="text-xs px-3 text-slate-500 max-w-[100px] truncate italic">{r.drcRemarks || "â€”"}</TableCell>
            <TableCell className="px-3">
              <div className="flex gap-1.5 flex-wrap items-center">
                {/* Approve */}
                <Button size="sm" className="h-7 text-xs px-2.5 bg-violet-700 hover:bg-violet-800"
