...
// DRC Office — receives >₹25k requests forwarded by DR
// ACTIONS: Forward to DRC (R&C) only — no reject, no query
          ...
          <div className="bg-white/70 backdrop-blur-lg border border-cyan-200/60 rounded-xl p-6 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                Handles requests <strong className="text-cyan-600">&gt; ₹25,000</strong> forwarded by DR &nbsp;
                <span className="text-slate-400 font-mono text-[11px]">
                  DA → AR → DR → <strong className="text-cyan-600">DRC Office</strong> → DRC (R&C) → DRC → Director
                </span>
              </p>
            </div>
            ...
            <CardHeader className="border-b border-slate-100">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription className="mt-0.5">Only <strong>DR-approved</strong> requests above ₹25,000 appear here.</CardDescription>
                </div>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4 p-4 bg-cyan-50 rounded-xl border border-cyan-200">
                {[
                  ...
                  ["Project Type", selectedRequest.projectType], ["Invoice No.", selectedRequest.invoiceNumber || "—"],
                ].map(([label, val]) => (
                  <div key={label}>
              ...
              {canAct(selectedRequest) && (
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    placeholder="Forwarding notes (optional)…"
          ...
          <DialogFooter>
            ...
            {selectedRequest && canAct(selectedRequest) && (
              <Button onClick={handleForward} disabled={actionLoading} className="bg-cyan-600 hover:bg-cyan-700">
                {actionLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Forwarding…</> : "Forward to DRC (R&C) ➡"}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
      ...
      <TableBody>
        {requests.map(r => (
          <TableRow key={r.id} className={`border-b border-slate-100 hover:bg-slate-50/50 ${canAct(r) ? "bg-cyan-50/40" : ""}`}>
            ...
            <TableCell className="text-xs px-3 text-slate-500 max-w-[100px] truncate italic">{r.drRemarks || "—"}</TableCell>
