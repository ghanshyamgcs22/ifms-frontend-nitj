// pages/approvals/DRDashboard.tsx â€” WITH VIEW QUOTATION IN REJECTED TAB
...
const formatDate = (d: string) => !d ? "â€”" : new Date(d).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" });
    ...
    <Layout>
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="bg-white/70 backdrop-blur-lg border border-purple-200/60 rounded-xl p-6 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">Deputy Registrar &nbsp;<span className="text-slate-400 font-mono">DA â†’ AR â†’ <strong className="text-purple-600">DR</strong></span></p>
            </div>
            <Button onClick={fetchRequests} variant="outline" size="sm">Refresh</Button>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 flex items-center gap-3"><CheckCircle className="h-5 w-5 text-emerald-600 shrink-0"/><div><p className="text-sm font-semibold text-emerald-800">â‰¤ â‚¹25,000</p><p className="text-xs text-emerald-600">DR gives final approval</p></div></div>
            <div className="rounded-xl border border-cyan-200 bg-cyan-50/60 p-4 flex items-center gap-3"><ArrowRight className="h-5 w-5 text-cyan-600 shrink-0"/><div><p className="text-sm font-semibold text-cyan-800">&gt; â‚¹25,000</p><p className="text-xs text-cyan-600">Forwards to DRC Office â†’ DRC (R&C) â†’ DRC â†’ Director</p></div></div>
          </div>
          ...
          <div className="grid grid-cols-5 gap-3">
            {[
              ...
              { label: "Final Approved (â‰¤25k)",   value: approved.length,        color: "text-emerald-600", icon: <CheckCircle   className="h-4 w-4 text-emerald-500"/>, sub: "Closed" },
          ...
          <Card className="border border-slate-200/60 bg-white/70 backdrop-blur-lg shadow-lg">
            <CardHeader className="border-b border-slate-100">
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="relative flex-shrink-0"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400 pointer-events-none"/><Input placeholder="Search GP No., PI, purposeâ€¦" value={reqSearch} onChange={e => setReqSearch(e.target.value)} className="pl-8 h-8 w-56 text-xs border-slate-200"/></div>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              {loading ? (<div className="flex items-center justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-purple-400 mr-2"/><p className="text-slate-500 text-sm">Loadingâ€¦</p></div>) : (
                <Tabs defaultValue="low">
                  <div className="px-4 pt-3">
                    <TabsList className="bg-purple-50">
                      <TabsTrigger value="low">â‰¤ â‚¹25k â€” Approve ({lowValuePending.length})</TabsTrigger>
                      <TabsTrigger value="high">&gt; â‚¹25k â€” Forward ({highValuePending.length})</TabsTrigger>
                  ...
                  {/* âœ… Rejected tab â€” showFile=true */}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {pendingAction === "approve" && "Final Approval (DR â€” â‰¤ â‚¹25k)"}
              {pendingAction === "forward" && "Forward to DRC Office (DR â€” > â‚¹25k)"}
          ...
          {selectedRequest && (
            <div className="space-y-5">
              <div className="flex items-center gap-2 flex-wrap">
                <Badge className={isHighValue(selectedRequest) ? "bg-cyan-100 text-cyan-800" : "bg-emerald-100 text-emerald-800"}>{isHighValue(selectedRequest) ? "> â‚¹25k â€” DRC Chain" : "â‰¤ â‚¹25k â€” DR Final Approve"}</Badge>
              ...
              {selectedRequest.status === "rejected" && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-xl space-y-1.5">
                  <p className="text-xs font-bold text-red-700 flex items-center gap-1.5"><XCircle className="h-3.5 w-3.5"/>Rejected by {selectedRequest.rejectedAtStageLabel || selectedRequest.rejectedAtStage || "â€”"}</p>
              ...
              {isQueryPending(selectedRequest) && selectedRequest.latestQuery && (
                <div className="p-4 bg-amber-50 border border-amber-300 rounded-xl space-y-2">
                  <p className="text-xs font-semibold text-amber-800 flex items-center gap-1"><MessageSquare className="h-3.5 w-3.5"/>Query sent to PI â€” awaiting response</p>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-purple-50 rounded-xl border border-purple-200">
                {[["GP Number", selectedRequest.gpNumber], ["PI Name", selectedRequest.piName], ["Department", selectedRequest.department], ["Amount", formatAmount(selectedRequest.amount)], ["Project Type", selectedRequest.projectType], ["Invoice No.", selectedRequest.invoiceNumber || "â€”"]].map(([label, val]) => (
              ...
              {pendingAction && canAct(selectedRequest) && (
                <div className="space-y-1.5">
                  ...
                  <Textarea placeholder={pendingAction === "query" ? "Enter query for PIâ€¦" : pendingAction === "reject" ? "Reason for rejectionâ€¦" : pendingAction === "forward" ? "Forwarding notesâ€¦" : "Final remarksâ€¦"} value={remarks} onChange={e => setRemarks(e.target.value)} rows={3} className="border-purple-200 focus:border-purple-400"/>
                </div>
              )}
          ...
          <DialogFooter>
            ...
            {selectedRequest && canAct(selectedRequest) && pendingAction && (
              <>
                {pendingAction === "reject"  && <Button variant="destructive" onClick={handleAction} disabled={actionLoading}>{actionLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Rejectingâ€¦</> : "Confirm Reject"}</Button>}
                {pendingAction === "query"   && <Button onClick={handleAction} disabled={actionLoading} className="bg-amber-500 hover:bg-amber-600">{actionLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Sendingâ€¦</> : <><MessageSquare className="h-4 w-4 mr-1"/>Send Query to PI</>}</Button>}
                {pendingAction === "forward" && <Button onClick={handleAction} disabled={actionLoading} className="bg-cyan-600 hover:bg-cyan-700">{actionLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Forwardingâ€¦</> : "Forward to DRC Office"}</Button>}
                {pendingAction === "approve" && <Button onClick={handleAction} disabled={actionLoading} className="bg-purple-700 hover:bg-purple-800">{actionLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Approvingâ€¦</> : "Final Approve"}</Button>}
              </>
            )}
          </DialogFooter>
        </DialogContent>
      ...
      <TableBody>
        {requests.map(r => (
          <TableRow key={r.id} className={`border-b border-slate-100 hover:bg-slate-50/50 ${highValue ? "bg-cyan-50/40" : "bg-purple-50/40"}`}>
            ...
            <TableCell className="text-xs px-3 text-slate-500 max-w-[90px] truncate italic">{r.daRemarks || "â€”"}</TableCell>
            <TableCell className="text-xs px-3 text-slate-500 max-w-[90px] truncate italic">{r.arRemarks || "â€”"}</TableCell>
      ...
      <TableBody>
        {requests.map(r => (
          <TableRow key={r.id} className="border-b border-amber-100 hover:bg-amber-50/50 bg-amber-50/30 cursor-pointer" onClick={() => onView(r)}>
            ...
            <TableCell className="text-xs px-3 text-slate-600 max-w-[180px] truncate italic">"{r.latestQuery?.query || "â€”"}"</TableCell>
...
// âœ… Dedicated rejected table with file number + view quotation
        ...
        {requests.map(r => (
          <TableRow key={r.id} className="border-b border-red-100 hover:bg-red-50/30 bg-red-50/10">
            ...
            <TableCell className="text-xs px-3 text-slate-600 italic max-w-[150px]">
              <p className="line-clamp-2">{r.rejectionRemarks || r.drRemarks || "â€”"}</p>
            </TableCell>
            {/* âœ… View quotation file */}
            <TableCell className="px-3">
              <button
