...
const DADashboard = () => {
  ...
  const fetchRequests = async () => {
    try {
      setLoadingReq(true);
      const r = await fetch(`${API}/get-budget-requests.php?type=all&summary=1&limit=50`);
  ...
  const handleView = async (r: BudgetRequest) => {
    setSelectedReq(r); // Set summarized data immediately
    setDialogOpen(true);
    setRemarks("");

    // Fetch full details (history, remarks, material, description etc.) which are excluded in summary mode
    try {
      const res = await fetch(`${API}/get-budget-requests.php?requestId=${r.id}`);
      const data = await res.json();
      if (data.success && data.data && data.data.length > 0) {
        setSelectedReq(data.data[0]);
      }
    } catch (e) {
                ...
                <Tabs defaultValue="myturn">
                  ...
                  <TabsContent value="myturn" className="mt-0 pt-3">
                    {loadingReq ? <Spinner /> : filterReqs(myTurnRequests).length === 0
                      ...
                      : <ReqTable requests={filterReqs(myTurnRequests)} canProcess={canProcess} onView={handleView} />}
                  </TabsContent>
                  <TabsContent value="forwarded" className="mt-0 pt-3">
                    {loadingReq ? <Spinner /> : filterReqs(forwardedRequests).length === 0
                      ...
                      : <ReqTable requests={filterReqs(forwardedRequests)} canProcess={() => false} onView={handleView} />}
                  </TabsContent>
                  <TabsContent value="history" className="mt-0 pt-3">
                    {loadingReq ? <Spinner /> : filterReqs(completedRequests).length === 0
                      ...
                      : <ReqTable requests={filterReqs(completedRequests)} canProcess={() => false} onView={handleView} />}
                  </TabsContent>
                </Tabs>
              </CardContent>
            </Card>
