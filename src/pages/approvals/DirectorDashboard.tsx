...
const DirectorDashboard = () => {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const r1 = await fetch(`${API}/get-requests-by-stage.php?stage=director&type=pending&summary=1&limit=50`);
      const d1 = await r1.json();
      setPendingRequests(d1.data || []);

      const r2 = await fetch(`${API}/get-requests-by-stage.php?stage=director&type=completed&summary=1&limit=50`);
