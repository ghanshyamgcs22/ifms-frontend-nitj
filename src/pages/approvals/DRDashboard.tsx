...
const DRDashboard = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const r1 = await fetch(`${API}/get-requests-by-stage.php?stage=dr&type=pending&summary=1&limit=50`);
      const d1 = await r1.json();
      setMyTurnRequests(d1.data || []);

      // 2) Globally completed (approved / rejected)
      const r2 = await fetch(`${API}/get-requests-by-stage.php?stage=dr&type=completed&summary=1&limit=50`);
      ...
      const r3 = await fetch(`${API}/get-requests-by-stage.php?stage=dr&type=sentback&summary=1&limit=50`);
      const d3 = await r3.json();
      setSentBackRequests(d3.data || []);

      // 4) Forwarded by DR (status = dr_approved etc., not yet approved/rejected)
      const r4 = await fetch(`${API}/get-requests-by-stage.php?stage=dr&type=forwarded&summary=1&limit=50`);
  ...
  const forwarded = filterReqs(forwardedRequests);
