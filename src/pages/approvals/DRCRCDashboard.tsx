...
const DRCRCDashboard = () => {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const r1 = await fetch(`${API}/get-requests-by-stage.php?stage=drc_rc&type=pending&summary=1&limit=50`);
      const d1 = await r1.json(); setPending(d1.data || []);
      const r2 = await fetch(`${API}/get-requests-by-stage.php?stage=drc_rc&type=completed&summary=1&limit=50`);
  ...
  const openReq = async (r: BudgetRequest, a?: ActionType) => {
    ...
    try {
      ...
      if (data.success && data.data && data.data.length > 0) {
        ...
        setSel(full);
        setApprovalType((full.approvalType as ApprovalType) || "");
      }
    } catch (e) {
