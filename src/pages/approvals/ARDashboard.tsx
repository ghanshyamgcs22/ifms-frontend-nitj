...
import {
  ...
  FileText, CheckCircle2, CheckCircle, CornerUpLeft, BadgeCheck, AlertCircle, History
} from "lucide-react";
...
const ARDashboard = () => {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const r1 = await fetch(`${import.meta.env.VITE_API_URL}/get-requests-by-stage.php?stage=ar&type=pending&summary=1&limit=50`);
      const d1 = await r1.json();
      setPendingRequests(d1.data || []);

      const r2 = await fetch(`${import.meta.env.VITE_API_URL}/get-requests-by-stage.php?stage=all&type=completed&summary=1&limit=50`);
      const d2 = await r2.json();
      setCompletedRequests(d2.data || []);

      const r3 = await fetch(`${import.meta.env.VITE_API_URL}/get-requests-by-stage.php?stage=ar&type=sentback&summary=1&limit=50`);
      const d3 = await r3.json();
      setSentbackRequests(d3.data || []);
      // 4) Forwarded by AR (not yet approved/rejected)
      const r4 = await fetch(`${import.meta.env.VITE_API_URL}/get-requests-by-stage.php?stage=ar&type=forwarded&summary=1&limit=50`);
      const d4 = await r4.json();
      setForwardedRequests(d4.data || []);
  ...
  const openRequest = async (r: BudgetRequest, action?: ActionType) => {
    ...
    setRemarks(""); setPendingAction(action || null); setDialogOpen(true);
    
    // Fetch full details (history, remarks, etc.) which are excluded in summary mode
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/get-requests-by-stage.php?requestId=${r.id}`);
      const data = await res.json();
  ...
  const completedRejected = completedRequests.filter(r => r.status === "rejected");
  const forwardedFiltered = forwardedRequests.filter(r => r.status === "ar_approved" || (r as any).status === "dr_approved");
  const freshRequests    = pendingRequests.filter(r => r.status === "da_approved");
  const returnedRequests = pendingRequests.filter(r => r.status === "sent_back_to_ar");
                  ...
                  <div className="px-4 pt-3">
                    <TabsList className="bg-indigo-50">
                      ...
                      <TabsTrigger value="history" className="text-xs">
                        Processed ({completedApproved.length})
                      </TabsTrigger>
                      <TabsTrigger value="forwarded" className="text-xs">
                        Forwarded ({forwardedFiltered.length})
                      </TabsTrigger>
                      <TabsTrigger value="sentback" className="text-xs">
 Back to DA ({sentbackRequests.length})</TabsTrigger>
                    </TabsList>
                  </div>
                  ...
                  <TabsContent value="myturn" className="px-4 pb-4 mt-3">
                    {filterReqs(pendingRequests).length === 0 ? (
                      ...
                      <RequestsTable requests={filterReqs(pendingRequests)} canAct={canAct} onView={openRequest} />
                    )}
                  </TabsContent>
                    ...
                    <TabsContent value="history" className="mt-0 pt-3">
                      ...
                      ) : completedApproved.length === 0
                        ? <EmptyState icon={<History className="h-10 w-10" />} message="No finished requests" />
                    ...
                    <TabsContent value="forwarded" className="mt-0 pt-3">
                      {loading ? (
                        <div className="flex items-center justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-slate-300" /></div>
                      ) : forwardedFiltered.length === 0
                        ? <EmptyState icon={<ArrowRight className="h-10 w-10" />} message="No forwarded requests pending at next stages" />
                        : <RequestsTable requests={filterReqs(forwardedFiltered)} canAct={() => false} onView={openRequest} />}
                    </TabsContent>
